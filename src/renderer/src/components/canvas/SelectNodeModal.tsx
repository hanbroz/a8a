...
import { IcoX, IcoTrash } from '../Icon'
...
function parseConfig(raw: string): SelectConfig {
  try {
    ...
    return { selectedRowIndices: [], autoSelect: false }
  }
}
...
function buildOutputJson(rows: Record<string, unknown>[], selectedIndices: number[]): string {
  if (rows.length === 0 || selectedIndices.length === 0) return 'null'
  const item = rows[selectedIndices[0]]
  return item !== undefined ? JSON.stringify(item, null, 2) : 'null'
}
...
export default function SelectNodeModal({
  ...
  const [selectedRowIndex, setSelectedRowIndex] = useState<number | null>(
    initial.selectedRowIndices.length > 0 ? initial.selectedRowIndices[0] : null
  )
  ...
  const handleRunClick = () => {
    ...
    setInputJson(result)
    setInputError(false)
    if (parseTableData(result)) setViewMode('table')
  }
  ...
  const handleSave = async () => {
    ...
    const config: SelectConfig = {
      selectedRowIndices: selectedRowIndex !== null ? [selectedRowIndex] : [],
      autoSelect,
    }
    await onSave(node.id, label.trim() || 'Select', JSON.stringify(config))
    setSaving(false)
    onClose()
  }

  const outputJson =
    inputRows && selectedRowIndex !== null ? buildOutputJson(inputRows, [selectedRowIndex]) : 'null'
        ...
        <div className="dm-modal-inner">
          ...
          <div className="dm-body" ref={bodyRef}>
                      ...
                      <table>
                        ...
                        <tbody>
                          {inputRows.map((row, idx) => {
                            const isChecked = selectedRowIndex === idx
                            return (
                              <tr
                                key={idx}
              ...
              <div className="dm-pane-body dm-settings-body">
                ...
                <div className="dm-field">
                  ...
                  <span style={{ fontSize: 12, color: 'var(--text-2)' }}>
                    {selectedRowIndex !== null ? `${selectedRowIndex + 1}번째 행 선택됨` : '미선택'} / 전체 {totalRows}행
                  </span>
                </div>
                ...
                {viewMode === 'table' && selectedRowIndex !== null && (
            ...
            <div className="dm-pane" style={{ width: rightW, flexShrink: 0 }}>
              ...
              <div className="dm-pane-body">
                <textarea
                  className="dm-json-area dm-json-area-select-output"
                  value={outputJson}
                  readOnly
                  spellCheck={false}
                />
