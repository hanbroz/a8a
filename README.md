...
URL은 치환 가능한 변수 값을 반영한 전체 주소를 줄이지 않고 표시합니다. API 목록 행을 클릭하면 캔버스의 해당 API 모듈이 선택됩니다. 이 기능은 전체 API 흐름이 올바른 endpoint 순서로 구성되었는지 확인할 때 사용합니다.
...
6. macOS 파일을 제공해야 한다면 같은 버전 번호로 macOS 환경에서 `.dmg`와 `.zip` 파일을 빌드합니다.
7. 버전 변경 파일을 commit/push 합니다.
8. `vyyyy.MM.dd.HH.mm` 태그로 GitHub Release를 생성합니다.
9. 설치 파일, blockmap 파일, 포터블 실행 파일, macOS 파일을 Release asset으로 업로드합니다.
