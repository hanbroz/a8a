...
![a8a 워크플로우 캔버스 화면](screenshot/a8a-workflow-canvas.png)
...
BASE를 제외한 환경에는 `실행시 경고표시` 옵션을 켤 수 있습니다. 이 옵션이 켜진 환경에서 전체 캔버스 실행, 모듈 INPUT 미리보기 실행, API 테스트 실행을 시작하면 실행 전에 현재 환경명을 확인하는 경고창이 표시됩니다. 경고창에서 `경고를 표시하지 않음`을 체크하고 확인하면 해당 환경의 `실행시 경고표시` 옵션이 해제됩니다.
...
DATA OUTPUT의 문자열 값에는 INPUT 값을 참조하는 `[[ ]]` 표현식을 사용할 수 있습니다. 예를 들어 INPUT에 `{ "amount": 12 }`가 있으면 OUTPUT에 `{ "total": "[[amount * 10]]" }`을 저장해 실행 시 `{ "total": 120 }`으로 만들 수 있습니다. 표현식이 문자열 전체를 차지하면 계산 결과의 숫자, 불리언, 객체 타입을 보존하고, 문장 안에 포함하면 문자열로 치환합니다.
...
API 모듈은 INPUT이 여러 노드에서 합쳐지거나 배열 값을 포함하더라도 노드 실행당 HTTP 요청을 한 번만 보냅니다. 여러 건 처리가 필요하면 START 반복 실행으로 실행 단위를 명시적으로 나누어야 합니다.
...
END 리포트 설정에서는 `INPUT`, `OUTPUT`, `PRE REQUEST`, `POST RESPONSE`, `사용된 변수`를 보고서에 포함할지 선택할 수 있습니다. 기존 END 설정은 모두 포함된 상태로 동작하며, 체크를 해제한 항목은 이후 생성되는 HTML/Markdown 리포트에서 제외됩니다. 단, API 모듈의 URL, 요청 헤더, 요청 바디, 응답 바디는 API 실행 증적이므로 `INPUT` 또는 `OUTPUT` 체크 여부와 관계없이 항상 보고서에 포함됩니다.
...
실행 로그 영역의 오른쪽 버튼은 로그가 닫혀 있을 때 `열기`, 열려 있을 때 `닫기(ESC)`로 표시됩니다. 실행 로그가 열린 상태에서는 `Esc` 키로 로그 창을 닫을 수 있습니다.
...
- API 모듈 실행 로그에서는 실제 요청 URL, Header, Body, 응답 Body, cURL 명령어를 확인할 수 있습니다.
- API 모듈은 INPUT이 여러 개이거나 배열이어도 노드 실행당 실제 HTTP 요청을 한 번만 수행합니다.
- 현재 활성 환경에 `실행시 경고표시`가 켜져 있으면 실행 전에 확인 경고를 표시하고, 확인하기 전에는 실행하지 않습니다.
